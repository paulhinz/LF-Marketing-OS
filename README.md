...
- `run-marketing-impact-report` — **v0.3.0.** The Marketing Dashboard as a Word document with live data. Asks which LF project or foundation to run against (e.g. CNCF) and for which period, checks the LFX connector (required) and the HubSpot connector (optional — the run continues without it), pulls the live figures, and writes one document that mirrors the Marketing Dashboard prototype ([clickable prototype v3.1](https://paulhinz.github.io/LF-Marketing-OS-Dashboards-Prototypes/)) one page per view: All › Summary; the tabs under All — Paid, Social, Web, Direct, **Defined Campaigns** (every goal from the Marketing Plan with KPI, budget and timeline → the campaigns under it with KPIs → each tactic per Medium : Source with its KPI) and **Budget** (paid ads by medium type with results plus the other marketing spend tracked by the ED and Marketing Lead); the five business outcomes (Memberships, Events, Education, Audience, Adoption); campaign and source detail. Every metric is tagged DECISION / SIGNAL / WATCH and carries its source and as-of date; every section whose data has no connector stays in the report with a "Data not available, requires [x] connector" notice, and a coverage appendix lists what was filled. Built for Jen Royle-Jones' and Federica's CNCF dashboard requirements. Output is built on the LF Agent DOCS Template with the bundled lf-output-formatter engine. Command: "run the marketing impact report for CNCF". (v0.3.0)
