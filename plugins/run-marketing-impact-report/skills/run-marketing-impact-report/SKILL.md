...
Produce the Marketing Impact dashboard as a document, with live data, for one Linux Foundation project or foundation. The document mirrors the v3 clickable prototype one page per view: business outcomes first (All › Outcomes, Memberships, Events, Education, Audience, Adoption), then the mediums under All (Paid, Paid › Budget, Social, Web, Direct, Attribution), then Goals › Campaigns, a campaign detail page, and four source-detail pages. Every page opens with the decision question it answers.
...
3. **The structure never changes.** `references/report_catalog.json` fixes 18 pages and 190 items. Do not add, drop or rename items; fill what the connectors allow.
...
3. **Optional inputs**: the Quarterly Marketing Plan deck and/or Quarterly Campaign Brief (for goals, budgets and campaign definitions), and the 3–5 message tags to track for narrative pull-through (fall back to the project's Message Foundation if in the working folder). If the user has none, the goal, budget and campaign-definition items carry the notice `requires Quarterly Marketing Plan document`.
...
Gather: email OVERVIEW totals and weekly series; per-email stats classified by type (newsletter, invite/reminder, nurture, survey); list sizes and 90-day active share; attribution by `UTM_MEDIUM` under LINEAR, FIRST_INTERACTION, LAST_INTERACTION and TIME_DECAY; closed-won and open membership deals; contacts created in the period and engaged in the last 90 days; content analytics TOTALS for HubSpot-hosted pages (views, visits, submissions, CTA views/clicks, entrances, exits, bounce rate, time per pageview); active campaigns and their analytics.
...
2. Read the builder's coverage line (`N/190 items filled`) and open the appendix table; confirm every LFX-sourced item that the connector could serve is filled, and that no notice appears for an item you have data for.
