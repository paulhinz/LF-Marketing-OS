...
description: >
  This skill should be used when a Linux Foundation or CNCF marketing leader, Executive
  Director, community manager, or anyone on their team says "build the ambassador content
  plan", "match ambassadors to the KubeCon schedule", "create ambassador content assignments
  for [event]", "run the KubeCon ambassador content agent", "generate ambassador social
  kits", or otherwise asks to assign event sessions, keynotes, or activities to ambassadors
  and produce personalized content (video prompts, social copy, hashtags, tags, posting
  times) for an LF or CNCF event. This is the LFX Marketing OS "KubeCon Ambassador Content
  Agent" planning phase. Produces an assignment tracker spreadsheet, per-ambassador content
  brief documents, and ready-to-send outreach drafts.
metadata:
  version: "0.1.0"
...
Ask (in one round, skipping anything already known from the conversation):

1. Which event? (e.g., "KubeCon + CloudNativeCon NA 2026"). Get the Sched.com URL if the user has it (e.g., `https://kccncna2026.sched.com`).
2. Which project/foundation and ambassador group? (e.g., CNCF Ambassadors).
