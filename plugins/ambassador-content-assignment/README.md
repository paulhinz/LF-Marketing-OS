# Ambassador Content Agent

An LFX Marketing OS agent for foundation and project marketing teams. It works for any LF project or foundation: it first asks which community the run is for, then matches that community's ambassadors against an event schedule (KubeCon, OSCon, or any LF/project event), creates personalized content assignments with video prompts, social copy, hashtags, tags, and posting recommendations, and tracks who actually posts.
...
Trigger with phrases like "build the ambassador content plan for KubeCon NA 2026" or "match our ambassadors to the event schedule". It first asks which community (project/foundation and ambassador group) the run is for and which event. It pulls the ambassador roster from an LFX committee, fetches the event schedule from Sched.com (with a browser fallback and an upload fallback), scores every ambassador-session pair, and produces three deliverables: an assignment tracker spreadsheet, a personalized content brief document per ambassador, and ready-to-send outreach drafts.
