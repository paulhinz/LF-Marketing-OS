...
14. **No dashes or middle dots.** Do not use em dashes or en dashes, including in
    ...
    Before: The build is fast, it finishes in 10 to 20 seconds (joined by a dash).
...
16. **Use straight quotes, not curly quotes.**
    Before: The system logs each "event" as it happens (curly quotes).
    After: The system logs each "event" as it happens (straight quotes).
...
file in the working output location for this session, e.g.,
`revision-<short-name>.html`. Do not write it into the skill folder. Check that
no `__DATA__` text remains in the saved file. Tell the user where the file is.
