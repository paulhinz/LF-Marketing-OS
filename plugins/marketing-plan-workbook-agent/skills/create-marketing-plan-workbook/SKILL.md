...
2. **Part II — The Goals**: five ranked goals, each with a deep-dive; explicit rule that the lower-numbered goal wins conflicts; retired goals named.
3. **Part III — Message & Audiences**: one messaging house mapped to goals (positioning, key message, proof, sound bite per goal); say/avoid/anchor discipline rules; segment table mapping audiences to goals.
4. **Part IV — One Plan, One Engine**: today's fragmentation (teams/calendars/reports), the integration fix (shared message/calendar/reporting), H2 anchor-campaign calendar, content engine arcs, amplification mechanics, media mix.
5. **Part V — Execution**: continuity with current half, team & budget, decisions needed with dates.

Every foundation gets this exact skeleton, regardless of size, for comparability.

## Workflow

### Step 1 — Identify the foundation

If not given, ask which foundation/project. Confirm the plan year/period (default: next calendar year).

### Step 2 — Pull the data (research before building)

Gather from three sources. If a source is unavailable (connector not authorized, file not found), note the gap, proceed, and convert missing data into questions in the workbook.

**a. Master foundation spreadsheet** — Google Sheet ID `110bv8q58jjcmRTpZe0meIcR8UuUisJd0FN3lnR_OQMg` ("120 Membership model projects"). Read via the Google Drive connector and extract the foundation's row: ED/PL name, MarCom staff, Events staff, PM/Community, renewal date, SOW status, existing Marketing Plan name/link, H2 Plan Updates, Marketing Budget, Event Marketing Budget, sub-project count, member count, revenue rank, LF Events calendar entries, and notes.

**b. Past marketing plan(s)** — Using the plan name/link found in the master spreadsheet, search Google Drive (`search_files`) for the foundation's current marketing plan and any H2 update, and read them. Extract: last period's goals and outcomes, messaging/positioning used, audience segments, campaign calendar, budget actuals, team, and anything flagged as retired or unresolved.
...
`[Foundation] [Year] Marketing Plan Workbook — DRAFT v1.pptx`

**Workbook conventions (apply throughout):**
- **Draft everything.** For every section, write a best-guess draft from the pulled data so the leader edits rather than writes. Tag each drafted slide with a visible badge: `DRAFT — CONFIRM OR CORRECT`.
...
- Never fabricate metrics. Draft ≠ invented: drafts must trace to a source or be phrased as a proposal ("proposed target: …").
